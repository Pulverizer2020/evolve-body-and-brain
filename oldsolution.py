  ...
  def Mutate(self):
    ...
    for i in self.body_builder.genotype.nodes:
      if np.random.rand() > 0.01:
        # NEED TO CHECK THIS ISN'T < 0
        self.body_builder.genotype.nodes[i]["length"] += np.random.normal(0,0.1)
      if np.random.rand() > 0.01:
        # NEED TO CHECK THIS ISN'T < 0
        self.body_builder.genotype.nodes[i]["width"] += np.random.normal(0,0.1)
      if np.random.rand() > 0.01:
        # NEED TO CHECK THIS ISN'T < 0
        self.body_builder.genotype.nodes[i]["height"] += np.random.normal(0,0.1)
      if np.random.rand() > 0.01:
        # HAVE TO UPDATE NEURON WEIGHTS IF has_sensor IS NOW TRUE
      ...
      # edges
      for u,v in self.body_builder.genotype.edges:
        # NEED TO CHECK THAT AT LEAST 1 IS == 0
        if np.random.rand() > 0.01:
          # NEED TO CHECK THIS ISN'T < 0
          self.body_builder.genotype.edges[(u,v)]["length_proportion"] += np.random.normal(0,0.1)
        if np.random.rand() > 0.01:
          # NEED TO CHECK THIS ISN'T < 0
          self.body_builder.genotype.edges[(u,v)]["width_proportion"] += np.random.normal(0,0.1)
        if np.random.rand() > 0.01:
          # NEED TO CHECK THIS ISN'T < 0
          self.body_builder.genotype.edges[(u,v)]["height_proportion"] += np.random.normal(0,0.1)
      ...
      if np.random.rand() > 0.01:
        ...
        nx.set_node_attributes(self.body_builder.genotype, new_node_id, {new_node_id: generateRandomNodeAttributes(genotype=self.body_builder.genotype)})
